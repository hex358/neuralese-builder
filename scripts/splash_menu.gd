...
func _ready() -> void:
	...
	if inner:
		ui.blur.modulate.a = 1.0
	else:
		ui.blur.modulate.a = 0.0
	show()
...
func _process(delta: float) -> void:
	...
	if $ColorRect.modulate.a <= 0.05 and not splashed:
		...
		show()
		var k = 1.0 if ui.blur.modulate.a < target_mod else 2
		$ColorRect.modulate.a = lerp($ColorRect.modulate.a, target_mod, delta * 20.0*k)
		if can_go:
			if not inner or !splashed:
				ui.blur.modulate.a = lerp(ui.blur.modulate.a, target_mod, delta * 20.0)
			else:
				ui.blur.modulate.a = 1.0
		else:
			ui.blur.modulate.a = 1.0
