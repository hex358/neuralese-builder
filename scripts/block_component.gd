...
var can_align: bool = true
func resize(_size: Vector2, set_base_size: bool = true, set_pos: bool = true) -> void:
	_size = _size.floor()
	if set_base_size:
		base_size = _size 
	size = _size; 
	...
	if button_type == ButtonType.BLOCK_BUTTON or button_type == ButtonType.DROPOUT_MENU:
		if not Engine.is_editor_hint():
			if set_pos:
				scaler.position.y = alignment.y*size.y
				position.y = -alignment.y*size.y
			scaler.position.x = alignment.x*size.x
...
func _align_label() -> void:
	...
	if expand_to_text_length and can_align:
		var full_sz: Vector2 = (font.get_string_size(
			txt,
		...
		var target_w = ceil(
			full_sz.x+abs(text_offset.x) * 2
			+ arrangement_padding.x * 2.0
		)
		...
		resize(new_size, true, _wrapped_in != self)
			

	var full_sz: Vector2 = font.get_string_size(txt, HORIZONTAL_ALIGNMENT_LEFT, -1.0, fs)
	var keep_txt = txt.substr(0, min(n, max(0, resize_after if !expand_to_text_length else 0)))
